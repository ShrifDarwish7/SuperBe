...
//        productsTableView.register(nib, forCellReuseIdentifier: CartProductTableViewCell.identifier)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let photos = items![indexPath.row].photos{
            ...
            cell.removeBtn.addTarget(self, action: #selector(removeItem(sender:)), for: .touchUpInside)
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let contextualAction = UIContextualAction(style: .destructive, title: "Delete".localized) { (_, _, _) in
    ...
    @objc func removeItem(sender: UIButton){
        CartServices.shared.removeItemAt(self.items![sender.tag]) { (completed) in
            self.fetchCartBranches()
            self.fetchCartItems()
        }
    }

    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let variations = lineItem?.variations, !variations.isEmpty{
            ...
            variations.forEach { (variation) in
                ...
                desc += (( "lang".localized == "en" ? "\(variation.name?.en ?? ""): " : "\(variation.name?.ar ?? ""): " ) + (variation.options?.map({ "lang".localized == "en" ? "\($0.name!.en ?? "")" : "\($0.name!.ar ?? "")" }).joined(separator: " , "))! + "\n")
            }
            cell.desc.text = desc
        }else{
        ...
        cell.price.text = "\(lineItem?.linePrice ?? 0) EGP"
